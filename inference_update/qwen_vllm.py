...
def generate(model_name,data_path,batch,max_new_tokens,save_name,lora_path):
    ...
    for i in tqdm(range(num)):
        ...
        for j in range(i*batch, (i+1)*batch):
            ...
            # message = [data[j]['messages'][0], data[j]['messages'][1]]
            # message[1]['content']  = message[1]['content'][:-1] + '? Only provide the product names, separated by ||, with no other response.'
            # message = data[j]['instruction']+ ' Only provide the product names, separated by ||, with no other response.'
            # message = data[j]['instruction']
        ...
        responses = llm.generate(prompts=inputs,use_tqdm = False, sampling_params=sampling_params,lora_request=LoRARequest("sql_adapter", 1, lora_path))
        res = []
    ...
    if num*batch != all_len:
        ...
        responses = llm.generate(prompts=inputs,use_tqdm = False, sampling_params=sampling_params,lora_request=LoRARequest("sql_adapter2", 2, lora_path))
        res = []
        for response in responses:
            res.append(response.outputs[0].text)
        with open(save_name, 'a') as f:
